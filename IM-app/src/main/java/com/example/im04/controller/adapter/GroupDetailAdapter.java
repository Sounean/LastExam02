...
public class GroupDetailAdapter extends BaseAdapter {
    ...
    private OnGroupDetailListener mOnGroupDetailListener;   //加减号的处理
    ...
    public void refresh(List<UserInfo> users){
        if (users != null && users.size() >=0){
            ...
            // 添加加号和减号(我们把放在最后面的加号和减号也做成一个userInfo)
            initUsers();
    ...
    private void initUsers() {
        ...
        mUsers.add(delete);//在集合中首先添加了减号，在集合中就是0号的索引
        mUsers.add(0 , add);// add添加进来占据了0的位子，delete就变成了1  每个都这么添加就保证了末尾是加号减号这样子
    }
